...
state read_inputfile(int argc, char* argv[], char* inputfile)
{
   if(argc != TWO_ARGUMENTS) {
      ...
      ON_ERROR("Correct Format: ./extension 3_test_files/filename.txt\n");
   }
...
state instruction(parse* data)
{
   if(move_forward(data) || rotate_left(data) || rotate_right(data) ||
      set(data) || do_loop(data) || draw_rectangle(data) || draw_arc(data)) {
      return pass;
   }
   return error;
}
...
state draw_arc(parse* data)
{
   ...
   if (strcmp(data->string_board[data->programme_cnt], ARC) == 0) {
      data->programme_cnt = data->programme_cnt + NEXT_LINE;
      if (!varnum(data->string_board[data->programme_cnt])) {
         ERROR("draw_arc function: Fatal error expecting <VARNUM> for angle",
                              data->string_board[data->programme_cnt]);
      }
      angle = set_variable(data);
      data->programme_cnt = data->programme_cnt + NEXT_LINE;

      if (!varnum(data->string_board[data->programme_cnt])) {
         ERROR("draw_arc function: Fatal error expecting <VARNUM> for radius",
                              data->string_board[data->programme_cnt]);
      }
      radius = set_variable(data);
      data->programme_cnt = data->programme_cnt + NEXT_LINE;

      set_arc(data, angle, radius);
...
{
   float current_angle, next_angle;
   int x;

   current_angle = data->coords.rotation;
   next_angle = angle * M_PI/M_ANGLE + current_angle;

   for(x = data->coords.rotation; data->coords.rotation < next_angle;
                                 data->coords.rotation += ANGLE_STEP) {
      get_next_coord(data, (radius * ANGLE_STEP));
   }
}
...
state draw_rectangle(parse* data)
{
   ...
   if (strcmp(data->string_board[data->programme_cnt], RECT) == 0) {
      data->programme_cnt = data->programme_cnt + NEXT_LINE;
      if (!varnum(data->string_board[data->programme_cnt])) {
         ERROR("draw_rectangle function: Fatal error expecting <VARNUM>",
                              data->string_board[data->programme_cnt]);
      }

      value = set_variable(data);
      set_rectagle(data, value);
...
{
   int x;
   for(x = 0; x < RECTANGLE_SIDES; x++) {
      data->coords.rotation = data->coords.rotation - TURN_ANGLE *
                                                   M_PI / M_ANGLE;
      get_next_coord(data, value);
   }
}
...
void get_next_coord(parse* data, float value)
{
   ...
   data->coords.x_coord[old][input_coords] +
   value * cos(data->coords.rotation);
   ...
   data->coords.y_coord[old][input_coords] +
   value * sin(data->coords.rotation);
}
